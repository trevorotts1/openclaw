...
describe("resolveMessagingTarget (directory fallback)", () => {
  ...
  it("does not apply the phone-number error to non-telegram phone-shaped targets", async () => {
    // The phone-number branch is telegram-gated; other channels (e.g. the
    // imessage/bluebubbles plugin resolver path) still resolve phone-shaped
    // inputs through their own plugin resolver exactly as before.
    mocks.getChannelPlugin.mockReturnValue({
      directory: {
        listPeers: mocks.listPeers,
        listPeersLive: mocks.listPeersLive,
    ...
    expect(result.target.resolutionSource).toBe("plugin");
  });
});
